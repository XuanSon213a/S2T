...
public class Gundam extends Sprite {
    ...
    public Gundam(BufferedImage image, int frameHeight, int frameWidth) {
    ...
    void addBullet(Gundam parent, int type) throws IOException {
		/*
		 *
    ...
    public void update() {
        ...
        if (this.currentHealth <= 0) {
            ...
            if (this instanceof AIMonster) ((Gundam1) MainCanvas.tankArray.get(0)).addScore(10);
        }
        if (this.currentHealth <= 0) {
            this.isDestroyed = true;
            if (this instanceof AIMonster) ((Gundam2) MainCanvas.tankArray2.get(0)).addScore(10);
        }
    ...
    public boolean checkCollisionWithBullets(Sprite a) {
        ...
        for (i = 0; i < MainCanvas.tankArray.size(); i++) {
            if (i != MainCanvas.tankArray.indexOf(a)) {
                ...
                Gundam tmp = MainCanvas.tankArray.get(i);
                for (j = 0; j < tmp.bulletArray.size(); j++) {
                    Bullet bullet = tmp.bulletArray.get(j);
                    if (a instanceof Gundam1
                            && bullet.getParent() instanceof AIMonster
                    ...
                    if (a instanceof AIMonster
                            && bullet.getParent() instanceof Gundam1
                            && MainCanvas.t.isCollision(a, bullet)) {
                        bullet.makeExplosion();
                        this.receivedDamage += bullet.getDamage();
    ...
    public boolean checkCollisionWithBullets2(Sprite a) {
        ...
        for (i = 0; i < MainCanvas.tankArray2.size(); i++) {
            if (i != MainCanvas.tankArray2.indexOf(a)) {
                // System.out.println("i = " + i + ", This = " +
                // MainCanvas.tankArray.indexOf(a));
                Gundam tmp = MainCanvas.tankArray2.get(i);
                for (j = 0; j < tmp.bulletArray.size(); j++) {
                    Bullet bullet = tmp.bulletArray.get(j);
                    if (a instanceof Gundam2
                            && bullet.getParent() instanceof AIMonster
                            && MainCanvas.t.isCollision(a, bullet)) {
                        bullet.makeExplosion();
                        this.receivedDamage += bullet.getDamage();
                    ...
                    if (a instanceof AIMonster
                            && bullet.getParent() instanceof Gundam2
                            && MainCanvas.t.isCollision(a, bullet)) {
                        bullet.makeExplosion();
                        this.receivedDamage += bullet.getDamage();
                        //System.out.println("AITank hited !!!");

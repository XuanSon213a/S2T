...
public class Item extends Sprite {
	...
	public void applyEffect(Gundam a) {
		switch (this.type) {
		case BIG_BULLET:
		...
		case CLOCK:
			if (a instanceof Gundam1) {
				for (int i = 0; i < MainCanvas.tankArray.size(); i++) {
					if (MainCanvas.tankArray.get(i) instanceof AIMonster) {
						((AIMonster) MainCanvas.tankArray.get(i)).setFrezzed(true);
					}
				}
			}
			if (a instanceof Gundam2) {
				for (int i = 0; i < MainCanvas.tankArray2.size(); i++) {
					if (MainCanvas.tankArray2.get(i) instanceof AIMonster) {
						((AIMonster) MainCanvas.tankArray2.get(i)).setFrezzed(true);
					}
				}
			}
			t = new Timer();
		...
		{
			if (a instanceof Gundam1) {
				((Gundam1) a).setLives(((Gundam1) a).getLives() + 1);
			}else if(a instanceof Gundam2) {
				((Gundam2) a).setLives2(((Gundam2) a).getLives2() + 1);
			}
			break;
		}

		case BOMB:
			if (a instanceof Gundam1 && a instanceof Gundam2) {
				for (int i = 0; i < MainCanvas.tankArray.size(); i++) {
					if (MainCanvas.tankArray.get(i) instanceof AIMonster) {
	...
	class ToDoTank extends TimerTask {
		...
		public ToDoTank(Item x, Gundam y) {
			this.item = x;
			this.tank = y;
		}
		...
		public void run() {
			...
			case CLOCK:
				for (int i = 0; i < MainCanvas.tankArray.size(); i++) {
					if (MainCanvas.tankArray.get(i) instanceof AIMonster) {
						((AIMonster) MainCanvas.tankArray.get(i))
								.setFrezzed(false);
					}
				}
				for (int i = 0; i < MainCanvas.tankArray2.size(); i++) {
					if (MainCanvas.tankArray2.get(i) instanceof AIMonster) {
						((AIMonster) MainCanvas.tankArray2.get(i))
								.setFrezzed(false);
					}
				}
				break;
